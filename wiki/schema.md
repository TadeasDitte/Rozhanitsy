...
whole request's inventory, not just one product's ranges. Attribution then
prefers components that satisfied a *bounded* clause (a real version range)
over ones that only satisfied an unbounded "this platform is present, any
version" clause, since the latter is how NVD expresses "plugin X requires
WordPress", not "WordPress itself is vulnerable" — falling back to every
satisfying component only if a satisfied group has no bounded clause at all.
Comparison itself is pure in-memory `VersionComparator`.
